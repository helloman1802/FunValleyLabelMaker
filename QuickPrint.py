...
sheet = sh.worksheet("Sheet3")
#app = Flask(__name__)

#@app.route('/')
def fastest_print_youve_seen():
    ...
    try:
        row_number = 2
        for row in range(end_row-2):
            if int(sheet.cell(row+2, 6).value) > 0:
                ...
                A = sheet.cell(row+2, 1).value
                # Discription
                B = sheet.cell(row+2, 2).value
                # Color
                C = sheet.cell(row+2, 3).value
                # Size
                D = sheet.cell(row+2, 4).value
                # Price
                E = sheet.cell(row+2, 5).value
                # Quantity
                F = sheet.cell(row+2, 6).value
                # Vendor SKU
                G = sheet.cell(row+2, 7).value
                # Vendor Invoice
                H = sheet.cell(row+2, 8).value
                # Vendor Name
                I = sheet.cell(row+2, 9).value
                # Circle or Square label
                J = sheet.cell(row+2, 10).value
        ...
        for index, row in data_frame_square.iterrows():
            print(int(totalLabels), int(row_count))
            print("New Product")
            for i in range(int(row["F"])):
